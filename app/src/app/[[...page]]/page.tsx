...
export default async function Home({ params, searchParams }: { params: { page: string; }, searchParams: { language: string } }) {
	...
	const translation = Object.entries(await client.getPage(params.page, searchParams.language))
		.map(([key, value]) => ({ key, value }));
	let defaultTranslation = undefined;
	if (defaultLanguage && searchParams.language !== defaultLanguage.code) {
		defaultTranslation = Object.entries(await client.getPage(params.page, defaultLanguage.code))
			.map(([key, value]) => ({ key, value }));
	}
	...
	async function saveLiterals(literals: FormLiteral[]) {
		'use server';
		return await client.saveLiterals(params.page, searchParams.language, literals);
	}
